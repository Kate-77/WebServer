...
std::string Response::createPath(const std::string &path) {
...
std::string Response::constructFilePath(const std::string &path) {
    ...
    if (locationPath->getLocationPath() != "/") {
        file.replace(0, locationPath->getLocationPath().length(), createPath(locationPath->getLocationRoot()+ "/"));
    }
